...
<AccordionGroup>
    <Accordion title="口语陪练" defalutopen icon="volume">
        通过实时对话\+视频反馈，及时纠正用户发音错误，支持视频捕捉用户表情、识别物体、浏览文档。
    </Accordion>
    <Accordion title="实时翻译" defalutopen icon="language">
        支持多语言实时对话，自动识别语种，完成自然语言交互\+即时翻译，媲美专业陪同翻译。
    </Accordion>
    <Accordion title="面试模拟" defalutopen icon="person-chalkboard">
        AI可扮演面试官模拟真实面试场景，根据不同岗位需求与候选人条件智能匹配面试问题。
    </Accordion>
    <Accordion title="旅行导游" defalutopen icon="map-location">
